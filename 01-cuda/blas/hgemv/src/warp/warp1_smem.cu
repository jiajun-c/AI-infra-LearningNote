...
__global__ void warp1SharedKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C, size_t N,
                                 size_t K) {
    ...
    int A_smem_iter = div_ceil(K, THREADS_PER_BLOCK);
    for (int i = 0; i < A_smem_iter; ++i) {
...
size_t initWarp1Smem(size_t K) {
    ...
    HGEMV_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(warp1SharedKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}
void warp1Shared(half *A, half *B, half *C, size_t N, size_t K)  {
    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(div_ceil(N, WARPS_PER_BLOCK));
    static size_t smem_max_size = initWarp1Smem(K);

    warp1SharedKernel<<<grid, block, smem_max_size>>>(A, B, C, N, K);
}