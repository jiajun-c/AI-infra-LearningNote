...
__global__ void warp8SmemKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C, size_t N,
                                size_t K) {
...
size_t initWarp8Smem(size_t K) {
    ...
    HGEMV_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(warp8SmemKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void warp8Smem(half *A, half *B, half *C, size_t N, size_t K) {
    static size_t smem_max_size = initWarp8Smem(K);

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(div_ceil(N, COLS_PER_BLOCK));

    warp8SmemKernel<<<grid, block, smem_max_size>>>(A, B, C, N, K);
}