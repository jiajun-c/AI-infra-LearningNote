            ...
            Alpha alpha, Beta beta)
{
  ...
  Tensor tAsA = local_partition(sA, tA, threadIdx.x, Step<_1, X>{});                  

  Tensor tBgB = local_partition(gB, tB, threadIdx.x);                  
  Tensor tBsB = local_partition(sB, tB, threadIdx.x, Step<_1, X>{});             
...
int main(int argc, char** argv)
{
  ...
  double time_double_async = timer.seconds() / timing_iterations;
  printf("gemm_double_buffer (Async)     : [%6.1f] GFlop/s  (%6.4f)ms\n", gflops / time_double_async, time_double_async*1000);
