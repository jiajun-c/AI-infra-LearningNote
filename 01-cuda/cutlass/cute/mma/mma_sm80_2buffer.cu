...
// ... [原有 SharedStorage 和 gemm_device kernel 代码保持不变] ...
using copyType = UniversalCopy<uint128_t>;
using copyAsyncType = SM80_CP_ASYNC_CACHEALWAYS<uint128_t>; 
...
// ... [gemm_nt, gemm_tn, gemm_template, gemm 辅助函数保持不变] ...
        ...
        cudaStream_t stream = 0)
{
  ...
  TiledCopy copyA = make_tiled_copy(Copy_Atom<copyAsyncType, cute::half_t>{},
                                    Layout<Shape<_16,_8>,Stride<_8,_1>>{},  // Thr layout 16x8 k-major
                                    Layout<Shape< _1,_8>>{});               // Val layout  1x8 k-major
  TiledCopy copyB = make_tiled_copy(Copy_Atom<copyAsyncType, cute::half_t>{},
                                    Layout<Shape<_16,_8>,Stride<_8,_1>>{},  // Thr layout 16x8 k-major
        ...
        cudaStream_t stream = 0)
{
  ...
  TiledCopy copyA = make_tiled_copy(Copy_Atom<copyAsyncType, TA>{},
                                    Layout<Shape<_32,_8>>{}, // Thr layout 32x8 m-major
                                    Layout<Shape< _4,_1>>{});// Val layout  4x1 m-major
  TiledCopy copyB = make_tiled_copy(Copy_Atom<copyAsyncType, TB>{},
...
int main(int argc, char** argv)
{
  ...
  cublasOperation_t transA_cb = (transA == 'N') ? CUBLAS_OP_N : CUBLAS_OP_T;
  cublasOperation_t transB_cb = (transB == 'N') ? CUBLAS_OP_N : CUBLAS_OP_T;

  // For cuBLAS GemmEx:
  // Using CUDA_R_16F (half) inputs and outputs.
  // Using CUDA_R_16F compute type (half accumulation) to match the CuTe code's SM80 instruction.
  // Note: Standard high-performance usually uses CUDA_R_32F (float accumulation), but here we align with your CuTe kernel.
  ...
  __half alpha_cb = (__half)1.0f;
  __half beta_cb  = (__half)0.0f;

  // cuBLAS Warmup
  cublasGemmEx(handle, transA_cb, transB_cb, m, n, k,
               &alpha_cb, d_A.data().get(), A_type, ldA,
               d_B.data().get(), B_type, ldB,
               &beta_cb,  d_C.data().get(), C_type, ldC,
