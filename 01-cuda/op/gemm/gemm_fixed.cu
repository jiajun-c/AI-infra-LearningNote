...
// ===================== 原始版本（有 bug） =====================
// 存在多个 bug：smemC 未初始化、B 索引错误、C 索引错误、bank conflict 等
template<const int TILE_SIZE = 16>
__global__ void gemm_base_NT_buggy(float* A, float* B, float* C, int M, int N, int K) {
    int bidx = blockIdx.x;
    int bidy = blockIdx.y;
    ...
    __shared__ float smemC[TILE_SIZE][TILE_SIZE];

    float* A_ptr = A + bidx * TILE_SIZE * K;
    float* B_ptr = B + bidy * TILE_SIZE * K;  // Bug: 应该是 B + bidy * TILE_SIZE

    for (int i = 0; i < K; i += TILE_SIZE) {
        smemA[tidx][tidy] = A_ptr[tidx * K + tidy];  // Bug: 索引错误
        smemB[tidx][tidy] = B_ptr[tidx * K + tidy];  // Bug: 索引错误
        __syncthreads();
        for (int j = 0; j < TILE_SIZE; j++) {
            smemC[tidx][tidy] += smemA[tidx][j] * smemB[tidy][j];  // Bug: smemC 未初始化 + bank conflict
        }
    }
    __syncthreads();
    float* C_ptr = C + (bidx * TILE_SIZE) * N + bidy;
    C_ptr[tidx * K + tidy] = smemC[tidx][tidy];  // Bug: K 应该是 N
}
...
int main() {
    ...
    for (auto& cfg : configs) {
        ...
        for (auto& k : kernels) {
            ...
            printf("\n  %s: grid=(%d,%d), block=(%d,%d)\n",
                   k.name, grid.x, grid.y, k.block.x, k.block.y);
