...
传统IO其实本质上都是同步的，不管是read/write还是epoll其实本质上都需要CPU去参与，当数据准备好了后，CPU必须亲自参与数据从内核空间到用户空间的拷贝

## 2. AIO

AIO 

- 提交队列(Submission Queue): 应用程序填写IO控制块，通过`io_submit` 一次性提交给内核，提交后系统调用瞬间返回，应用程序可以继续执行其他计算
- 完成队列(Completion Queue): 当底层的硬件完成数据读取后，内核将会把结果放入到队列里，应用程序在需要数据的时候，通过`io_getevents`去获取结果
